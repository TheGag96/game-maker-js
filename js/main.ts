...
namespace Shell {
  ...
  export function init(canvas: HTMLCanvasElement) {
    ...
    function addEventToElement(name: string, event: string, callback) {
  ...
  function onCanvasMouseDown(event) {
    ...
    let lastSelected = editor.selected;
  ...
  function onCanvasMouseMove(event) {
    ...
    if (editor.dragging) {
      ...
      for (var i = 0; i < rowList.length; i++) {
        var row = rowList[i] as HTMLElement;

        if (row.tagName !== undefined && row.tagName.toLowerCase() === "div") {
          var propName = (row.firstElementChild as HTMLElement).innerText;
  ...
  if (game.isRunning()) {
      ...
      for (let ent of editor.entityList) {
  ...
  function onRemoveSpriteButtonClick(event) {
    if (!game.isStopped() || !editor.selected) return;
  ...
  function onFileNewButtonClick(event) {
    if (!game.isStopped() || !confirm("Shell will clear everything. Did you save your work?")) return;
   ...
   **/
  function makeRealEntity(ent): Entity {
